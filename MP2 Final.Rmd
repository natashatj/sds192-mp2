...
##FEC 2012 Data - Partisan Contributions Mapped To The United States
##The United States According To The Canadian, The Australian And The Kiwi.

When comparing both the Republican map and the Democrat map side by side, it is apparent that no matter the party identification, both parties seem to target the same states, such as California, Florida, New York, Illinois and Ohio. However, we notice that democrats invest more money than republicans into California as it is a typically blue state. They are guaranteed to take California in the electoral college. Republicans received a lot of money from Texans, which is not surprising because Texas is a conservative state. When observing the map of contributions for the Democrat Party, it is striking that Wyoming has no color attributed to it. This could potentially be the result of missing data or the unlikely lack of investment for Democrats in the region. Swing states have higher contribution rates than safe states. We also know that wealth is concentrated on the coasts which may explain why Californians also donated a lot of money to Republicans. In terms of visualization, the scales are different for each map. The Democrat map has a wider scale, comprised of five contribution variations, whilst the Republican map only has three variations. We can deduce that this is because the Democrats may have received more contributions. It would have been ideal to convert the sum of contributions into a currency variable, but in order to map currency variables, we would need to create bins to place them in - i.e $50,000-$100,000 and so on. It was easier to organise continuous variables and have ggplot place them onto a scale instead of manually sorting discrete variables ourselves. 
